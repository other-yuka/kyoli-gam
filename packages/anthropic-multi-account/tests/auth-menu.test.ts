  ...
  it("returns active when cached usage is available even if stale rateLimitResetAt is in the future", async () => {
    ...
    const account = {
      ...
      cachedUsage: {
        ...
        seven_day: { utilization: 39, resets_at: new Date(Date.now() + 86400_000).toISOString() },
        seven_day_sonnet: { utilization: 12, resets_at: new Date(Date.now() + 86400_000).toISOString() },
      },
    };
