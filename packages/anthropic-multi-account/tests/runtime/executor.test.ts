...
import { executeWithAccountRotation } from "../../src/runtime/executor";
import { createMockClient } from "../helpers";
import type { ManagedAccount, PluginClient } from "../../src/shared/types";
...
describe("executeWithAccountRotation", () => {
  ...
  test("returns response on first successful request", async () => {
    const manager = createMockManager([createAccount()]);
  ...
  test("on revoked 403 marks account revoked and rotates", async () => {
    ...
    const fetchFn = vi.fn(() => {
      callCount += 1;
      if (callCount === 1) {
        return Promise.resolve(jsonResponse({ error: "token has been revoked" }, 403));
      }
      return Promise.resolve(jsonResponse({ ok: true }));
    });
  ...
  test("clears OpenCode auth when all accounts are revoked or disabled", async () => {
    ...
    const fetchFn = vi.fn(() =>
      Promise.resolve(jsonResponse({ error: "token has been revoked" }, 403)));
