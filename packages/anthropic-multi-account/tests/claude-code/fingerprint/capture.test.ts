...
const fingerprintCaptureModulePath = "../../../src/claude-code/fingerprint/capture?fingerprint-capture-test";
const {
...
const DEFAULT_CAPTURED_CC_VERSION = getBundledCCVersion();

function getBundledCCVersion(): string {
  return loadTemplate().cc_version ?? "0.0.0";
}

function createBillingHeader(ccVersion = DEFAULT_CAPTURED_CC_VERSION, build = "bb5"): string {
  return `x-anthropic-billing-header: cc_version=${ccVersion}.${build}; cch=00000;`;
}

function createUserAgent(ccVersion = DEFAULT_CAPTURED_CC_VERSION): string {
...
describe("fingerprint-capture", () => {
  ...
  test("refreshLiveFingerprintAsync short-circuits fresh cache and refreshes stale cache", async () => {
    ...
    try {
      ...
      setFingerprintCaptureTestOverridesForTest({
        findClaudeBinary: () => "/mock/claude",
        detectCliVersion: () => bundled.cc_version ?? DEFAULT_CAPTURED_CC_VERSION,
        runClaudeCapture: async ({ baseUrl }) => {
          await fetch(`${baseUrl}/v1/messages`, {
            ...
            headers: {
              "anthropic-version": "2023-06-01",
              "content-type": "application/json",
              "user-agent": createUserAgent(bundled.cc_version ?? DEFAULT_CAPTURED_CC_VERSION),
              "x-app": "cli",
            },
            body: JSON.stringify({
              system: [
                createBillingHeader(bundled.cc_version ?? DEFAULT_CAPTURED_CC_VERSION, "abc"),
  ...
  test("loadTemplate keeps cached data even when cached version mismatches bundled version", async () => {
    ...
    try {
      setFingerprintCaptureTestOverridesForTest({
        detectCliVersion: () => getBundledCCVersion(),
      });
        ...
        runClaudeCapture: async ({ baseUrl }) => {
          await fetch(`${baseUrl}/v1/messages`, {
            ...
            headers: {
              "anthropic-version": "2023-06-01",
              "content-type": "application/json",
              "user-agent": `claude-cli/${DEFAULT_CAPTURED_CC_VERSION} (external, sdk-cli)`,
              "x-app": "cli",
            },
            body: JSON.stringify({
              system: [
                createBillingHeader(DEFAULT_CAPTURED_CC_VERSION),
