...
describe("anthropic-oauth", () => {
  ...
  describe("detectOAuthConfig", () => {
    ...
    test("silently falls back when the binary cannot be read", async () => {
      await withTempDir(async (dir) => {
        const ccPath = await writeTempFile(dir, "claude", "fake claude binary");
    ...
    test("never throws on malformed binary buffer", async () => {
      await withTempDir(async (dir) => {
        const ccPath = await writeTempFile(dir, "claude-malformed-bin", "malformed-test-binary");
    ...
    test("applies env override above detected config", async () => {
      await withTempDir(async (dir) => {
        const ccPath = await writeTempFile(dir, "claude-override-bin", "override-test-binary");
    ...
    test("normalizes authorizeUrl for fresh detected runtime config", async () => {
      await withTempDir(async (dir) => {
        const ccPath = await writeTempFile(dir, "claude-detected-bin", "detected-test-binary");
        ...
        expect(config.authorizeUrl).toBe(DEFAULT_AUTHORIZE_URL);
      });
    });
    ...
    test("applies file override when env override is absent", async () => {
      await withTempDir(async (dir) => {
        ...
        const overridePath = await writeTempFile(dir, "oauth-config.override.json", JSON.stringify({
    ...
    test("ignores cached entries with polluted Google scope URLs", async () => {
      await withTempDir(async (dir) => {
        const binaryContents = "fake claude binary";
        const ccPath = await writeTempFile(dir, "claude-polluted-cache-bin", binaryContents);
        const ccHash = createHash("sha256").update(binaryContents).digest("hex").slice(0, 16);
        await writeTempFile(dir, "anthropic-oauth-config-cache.json", JSON.stringify({
          entries: {
            [ccHash]: {
              clientId: DETECTED_CLIENT_ID,
        ...
        expect(config.source).toBe("detected");
        expect(config.scopes).toBe("scope:detected user:sessions:claude_code");
        expect(config.scopes).not.toBe("https://www.googleapis.com/auth/cloud-platform");
      });
    });
  });
  ...
  describe("normalizeAuthorizeUrl", () => {
    ...
    test("passes through arbitrary operator URLs unchanged (staging, localhost, custom)", () => {
      expect(normalizeAuthorizeUrl("https://staging.example.com/oauth/authorize"))
        .toBe("https://staging.example.com/oauth/authorize");
      expect(normalizeAuthorizeUrl("http://localhost:3000/oauth/authorize"))
        .toBe("http://localhost:3000/oauth/authorize");
      expect(normalizeAuthorizeUrl("https://claude.ai/oauth/authorize/extra"))
        .toBe("https://claude.ai/oauth/authorize/extra");
    });

    test("override containing legacy URL is normalized on detectOAuthConfig", async () => {
      process.env.ANTHROPIC_MULTI_ACCOUNT_OAUTH_AUTHORIZE_URL = LEGACY_AUTHORIZE_URL;
      setOAuthConfigDetectionOverridesForTest({
        findCCBinary: () => null,
      });
    ...
    test("loadCache normalizes legacy URLs in pre-existing cache entries", async () => {
      await withTempDir(async (dir) => {
        ...
        await writeTempFile(dir, "anthropic-oauth-config-cache.json", JSON.stringify({
          entries: { "pre-existing-hash": legacyCacheEntry },
          savedAt: Date.now(),
        }));

        const entries = await loadCache();

        expect(entries["pre-existing-hash"]?.authorizeUrl)
          .toBe(DEFAULT_AUTHORIZE_URL);
      });
    });

    test("saveCache preserves raw legacy URLs already on disk", async () => {
      await withTempDir(async (dir) => {
        const cachePath = `${dir}/anthropic-oauth-config-cache.json`;
        const legacyCacheEntry = {
          clientId: "11111111-1111-4111-8111-111111111111",
          authorizeUrl: LEGACY_AUTHORIZE_URL,
          tokenUrl: DEFAULT_TOKEN_URL,
        ...
        await saveCache("new-hash", {
          clientId: DETECTED_CLIENT_ID,
          authorizeUrl: DEFAULT_AUTHORIZE_URL,
          tokenUrl: DEFAULT_TOKEN_URL,
          scopes: "scope:detected user:sessions:claude_code",
          baseApiUrl: DEFAULT_BASE_API_URL,
        });
        ...
        expect(saved.entries?.["pre-existing-hash"]?.authorizeUrl)
          .toBe(LEGACY_AUTHORIZE_URL);
        expect(saved.entries?.["new-hash"]?.authorizeUrl)
          .toBe(DEFAULT_AUTHORIZE_URL);
      });
    });
  });
  ...
  test("postTokenEndpoint throws a debuggable invalid JSON error when the token body is malformed", async () => {
    ...
    const request = anthropicOAuthTestExports.postTokenEndpoint(
      "application/json",
      JSON.stringify({ grant_type: "refresh_token" }),
    );

    await expect(request).rejects.toThrow("Anthropic token request returned invalid JSON.");
    await expect(request).rejects.toThrow("body=not-json");
  });
  ...
  describe("refreshWithOAuth", () => {
    ...
    test("throws error containing HTTP status when token endpoint returns non-OK", async () => {
      ...
      const refresh = refreshWithOAuth("expired-refresh");

      await expect(refresh).rejects.toThrow("401");
      await expect(refresh).rejects.toThrow("Anthropic token request failed");
    });
  });
  ...
  describe("exchangeCodeForTokens", () => {
    ...
    test("throws an error with HTTP status when the token endpoint returns non-OK", async () => {
      ...
      const exchange = anthropicOAuthTestExports.exchangeCodeForTokens({
        code: "bad-code",
      ...
      await expect(exchange).rejects.toThrow("Anthropic token request failed");
    });
  });
  ...
  describe("loginWithOAuth", () => {
    test("completes the public OAuth login flow and returns Partial<StoredAccount>", async () => {
      ...
      expect(authorizeUrl.origin + authorizeUrl.pathname).toBe(DEFAULT_AUTHORIZE_URL);
